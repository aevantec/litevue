...
- `Map` and `Set` are not iterated. Spread one into an array — `v-for="[k, v] in [...map]"` — and note that changes to its contents are not tracked; see [reactive()](/globals/reactive#behavior).
